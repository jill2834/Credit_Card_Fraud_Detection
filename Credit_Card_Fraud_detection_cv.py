...
#os.chdir('/Users/Jill/Documents/Internship_Project/fraud_transaction_detection ')
...
log_fpr, log_tpr, log_thresold = precision_recall_curve(labels_log_reg, predictions_log_reg)
log_reg_auc = auc(log_tpr, log_fpr)

knear_fpr, knear_tpr, knear_threshold = precision_recall_curve(labels_knn, predictions_knn)
knear_auc = auc(knear_tpr, knear_fpr)

svc_fpr, svc_tpr, svc_threshold = precision_recall_curve(labels_svm, predictions_svm)
svc_auc = auc(svc_tpr, svc_fpr)

tree_fpr, tree_tpr, tree_threshold = precision_recall_curve(labels_tree, predictions_tree)
tree_auc = auc(tree_tpr, tree_fpr)

forest_fpr, forest_tpr, forest_threshold = precision_recall_curve(labels_forest,predictions_forest)
forest_auc = auc(forest_tpr, forest_fpr)

xgboost_fpr, xgboost_tpr, xgboost_threshold = precision_recall_curve(labels_xgboost, predictions_xgboost)
...
def graph_precision_recall_curve_multiple(log_fpr, log_tpr, knear_fpr, knear_tpr, svc_fpr, svc_tpr, tree_fpr, tree_tpr, forest_fpr, forest_tpr, xgboost_fpr, xgboost_tpr):
	plt.figure(figsize=(16,8))
	plt.title('Precision_Recall Curve \n Supervised Classifiers', fontsize=18)
	'''
    plt.step(log_fpr, log_tpr, alpha = 0.2, where = 'post')
	plt.step(knear_fpr, knear_tpr, alpha = 0.2, where = 'post')
	plt.step(svc_fpr, svc_tpr, alpha = 0.2, where = 'post')
	plt.step(tree_fpr, tree_tpr, alpha = 0.2, where = 'post')
	plt.step(forest_fpr, forest_tpr, alpha = 0.2, where = 'post')
	plt.step(xgboost_fpr, xgboost_tpr, alpha = 0.2, where = 'post')
    '''
    
	plt.plot(log_fpr, log_tpr, label='Logistic Regression Classifier AUC Score: {:.4f}'.format(log_reg_auc))
	plt.plot(knear_fpr, knear_tpr, label='KNears Neighbors Classifier AUC Score: {:.4f}'.format(knear_auc))
	plt.plot(svc_fpr, svc_tpr, label='Support Vector Classifier AUC Score: {:.4f}'.format(svc_auc))
	plt.plot(tree_fpr, tree_tpr, label='Decision Tree Classifier AUC Score: {:.4f}'.format(tree_auc))
	plt.plot(forest_fpr, forest_tpr, label='Random Forest Classifier AUC Score: {:.4f}'.format(forest_auc))
	plt.plot(xgboost_fpr, xgboost_tpr, label='Xgboost Classifier AUC Score: {:.4f}'.format(xgboost_auc))
	plt.axis([-0.01, 1, 0, 1])
	plt.xlabel('Recall', fontsize=16)
	plt.ylabel('Precision', fontsize=16)
	plt.legend()
	   
graph_precision_recall_curve_multiple(log_fpr, log_tpr, knear_fpr, knear_tpr, svc_fpr, svc_tpr, tree_fpr, tree_tpr, forest_fpr, forest_tpr, xgboost_fpr, xgboost_tpr)
plt.show()
...
graph_roc_curve_multiple(ae_fpr, ae_tpr, rbm_fpr, rbm_tpr)
plt.show()
